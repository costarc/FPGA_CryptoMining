...
class ProxyClientProtocol(protocol.Protocol):
    def connectionMade(self):
        ...
        sub1 = subscribe.decode()+'\n'
        self.cli_queue.put(bytes(sub1.encode('utf-8')))
        ...
        for val in curstr:
            ...
            try:
                ...
                if modifiedchunk is not None:				
                    modf1 = modifiedchunk+'\n'
                    self.factory.srv_queue.put(modf1.encode('utf-8'))   # send processed 
    ...
    def connectionLost(self, why):
        if self.cli_queue:
            self.cli_queue = None
            self.factory.srv_queue.put(bytes(('reconnect\n').encode('utf-8')))
...
class ProxyServer(protocol.Protocol):
    ...
    def dataReceived(self, chunk):
        ...
        try:
            if re.match(r'auth\s(.+)', schunk):
                self.cli_jsonid = 1
                modifiedchunk = self.doAuth(schunk)
            elif re.match(r'submit_nonce', schunk):
                match_obj = re.match(r'submit_nonce\s(\w+)\s(\w+)\s(\w+)\s(\w+)', schunk)
            ...
            modf1 = modifiedchunk.decode()+'\n' 
            self.cli_queue.put(bytes(modf1.encode('utf-8')))                     # send processed input
        except Exception as e:
            print(e)
